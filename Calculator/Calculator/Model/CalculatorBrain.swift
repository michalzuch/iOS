...
class CalculatorBrain {
    ...
    func inputNumber(_ number: Double) {
        if isDecimalEntered {
            currentOperand = (currentOperand ?? 0.0) + number * decimalMultiplier
    ...
    func performPendingOperation() {
        if let operation = currentOperation, let operand = currentOperand, let result = result {
